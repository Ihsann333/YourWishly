...
@section('title', 'Edit Kartu - YourWishly')
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10 relative">
    ...
    <div class="mb-8">
        ...
        <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight mt-2">Edit Kartu Ucapan</h1>
        <p class="text-slate-500 text-sm">Sesuaikan informasi atau ubah template kartu ucapan digital Anda.</p>
    </div>

    <form action="{{ route('cards.update', $card->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white p-8 rounded-[32px] border border-slate-100 shadow-sm space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" id="recipient_name" name="recipient_name" required value="{{ old('recipient_name', $card->recipient_name) }}"
                ...
                <div>
                    ...
                    <input type="text" id="sender_name" name="sender_name" required value="{{ old('sender_name', $card->sender_name ?: $card->user->name ?? '') }}"
                        class="appearance-none block w-full px-4 py-3 border border-slate-200 rounded-xl placeholder-slate-400 text-slate-900 focus:outline-none focus:ring-2 focus:ring-rose-500 focus:border-rose-500 text-sm transition-all"
                ...
                <div>
                    ...
                    <select id="event_type" name="event_type" required
                        ...
                        <option value="Ulang Tahun" {{ old('event_type', $card->event_type) == 'Ulang Tahun' ? 'selected' : '' }}>Ulang Tahun 🎂</option>
                        <option value="Wisuda" {{ old('event_type', $card->event_type) == 'Wisuda' ? 'selected' : '' }}>Wisuda 🎓</option>
                        <option value="Pernikahan" {{ old('event_type', $card->event_type) == 'Pernikahan' ? 'selected' : '' }}>Pernikahan 💍</option>
                        <option value="Hari Raya" {{ old('event_type', $card->event_type) == 'Hari Raya' ? 'selected' : '' }}>Hari Raya 🌙</option>
                        <option value="Anniversary" {{ old('event_type', $card->event_type) == 'Anniversary' ? 'selected' : '' }}>Anniversary ❤️</option>
                        <option value="Perpisahan" {{ old('event_type', $card->event_type) == 'Perpisahan' ? 'selected' : '' }}>Perpisahan 👋</option>
                    </select>
                    @error('event_type')
                        <p class="mt-1 text-xs font-semibold text-rose-500">{{ $message }}</p>
                    @enderror
            ...
            <div>
                ...
                <input type="text" id="title" name="title" required value="{{ old('title', $card->title) }}"
                    class="appearance-none block w-full px-4 py-3 border border-slate-200 rounded-xl placeholder-slate-400 text-slate-900 focus:outline-none focus:ring-2 focus:ring-rose-500 focus:border-rose-500 text-sm transition-all"
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="4"
                    class="appearance-none block w-full px-4 py-3 border border-slate-200 rounded-xl placeholder-slate-400 text-slate-900 focus:outline-none focus:ring-2 focus:ring-rose-500 focus:border-rose-500 text-sm transition-all"
                    placeholder="Tulis kalimat pembuka singkat sebelum kartu dibuka.">{{ old('description', $card->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-xs font-semibold text-rose-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea id="body" name="body" rows="7" required
                    ...
                    placeholder="Tulis isi utama kartu di sini: ucapan, doa, cerita singkat, detail acara, atau pesan spesial untuk penerima.">{{ old('body', $card->body) }}</textarea>
            ...
            <div>
                ...
                @if($card->photo_path)
                    <div class="mb-3 overflow-hidden rounded-2xl border border-slate-100 bg-slate-50 aspect-[4/3]">
                        <img src="{{ asset('storage/' . $card->photo_path) }}" alt="Foto kartu {{ $card->recipient_name }}" class="w-full h-full object-cover">
                    </div>
                @endif
                ...
                <p class="text-[10px] text-slate-400 mt-1">Upload foto baru kalau ingin mengganti foto kartu. Format JPG, PNG, WEBP. Maksimal 3MB. Rekomendasi rasio 4:3 atau 1:1 supaya hasilnya fit dan tetap rapi di kartu.</p>
                @error('photo')
                    <p class="mt-1 text-xs font-semibold text-rose-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="datetime-local" id="countdown_date" name="countdown_date" 
                    value="{{ old('countdown_date', $card->countdown_date ? $card->countdown_date->format('Y-m-d\TH:i') : '') }}"
                    class="appearance-none block w-full px-4 py-3 border border-slate-200 rounded-xl placeholder-slate-400 text-slate-900 focus:outline-none focus:ring-2 focus:ring-rose-500 focus:border-rose-500 text-sm transition-all">
                ...
                @foreach($templates as $template)
                    <label class="relative block cursor-pointer group">
                        <input type="radio" name="template_id" value="{{ $template->id }}" class="sr-only peer" 
                            {{ (old('template_id', $card->template_id) == $template->id) ? 'checked' : '' }}>
